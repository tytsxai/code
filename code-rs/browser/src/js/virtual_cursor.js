...
(function () {
  function __vcInstall(x, y) {
    try {
      ...
      function log() {
        try {
          console.debug("[VC]", ...arguments);
        } catch (e) {}
      }
      function warn() {
        try {
          console.warn("[VC]", ...arguments);
        } catch (e) {}
      }
      function info() {
        try {
          console.info("[VC]", ...arguments);
        } catch (e) {}
      }
      ...
      if (!window.__vc) {
        ...
        try {
          arrow.style.transformBox = "view-box";
        } catch (_) {}
        ...
        wrap.style.transform =
          "translate3d(" + state.wrapX + "px," + state.wrapY + "px,0)";
        // Arrow so its tip is at (0,0)
        arrow.style.transform =
          "translate3d(" + -TIP_X + "px," + -TIP_Y + "px,0)";
        // Badge positioned by its top-left offset from tip
        badge.style.transform =
          "translate3d(" + BADGE_TLX + "px," + BADGE_TLY + "px,0)";
        ...
        const MOTION = {
          ...
          backBegin: 0.4, // when to start rotating back (0..1)
        };
        ...
        function durationForDistance(d) {
          ...
          const t = Math.max(
            0,
            Math.min(1, (d - minDist) / (maxDist - minDist)),
          );
        ...
        function cssAnim(
          ...
          try {
            ...
            el.style.transform =
              ...
              Math.round(nextY) +
              "px,0)";
            if (which === "a") state.cssActiveA = true;
            else if (which === "b") state.cssActiveB = true;
            const onEnd = () => {
              if (which === "a") state.cssActiveA = false;
              else state.cssActiveB = false;
              el.removeEventListener("transitionend", onEnd);
            };
            el.addEventListener("transitionend", onEnd);
          } catch (e) {
            warn("cssAnim error", e);
          }
        }
        ...
        function moveTo(nx, ny, opts) {
          ...
          const wx1 = Math.round(nx),
            wy1 = Math.round(ny);
          const wx0 = state.wrapX,
            wy0 = state.wrapY;
          ...
          if (d < 1.5) {
            try {
              state.aAnim && state.aAnim.cancel();
            } catch (e) {}
            try {
              state.bAnim && state.bAnim.cancel();
            } catch (e) {}
            wrap.style.transform = "translate3d(" + wx1 + "px," + wy1 + "px,0)";
            try {
              arrowInner.style.transform = "translate(0px,0px) rotate(0deg)";
            } catch (_) {}
            commit(wx1, wy1, 0, 0);
          ...
          try {
            ...
            state.ignoreHoverUntil = Math.max(
              state.ignoreHoverUntil,
              now + totalPlan + 40,
            );
          ...
          const orbitScale = enableOrbit
            ? Math.max(0, Math.min(0.6, (d - 100) * 0.0006))
            : 0; // 0..0.6 (over 1000)
          ...
          const dur =
            o.cssDurationMs && o.cssDurationMs > 0 ? o.cssDurationMs | 0 : aDur;
          const retarget = sincePrev <= coalesceMs;
          if (retarget && (state.cssActiveA || state.cssActiveB)) {
            log("css retarget", {
              dur,
              sincePrev,
              cssActiveA: state.cssActiveA,
              cssActiveB: state.cssActiveB,
            });
            wrap.style.transform = "translate3d(" + wx1 + "px," + wy1 + "px,0)";
          } else {
          ...
          try {
            ...
            if (dx <= 0 && dy <= 0) {
              ...
              ORI_X = 0;
              ORI_Y = 0;
              cssDeg = Math.round(0 * qScale);
            } else if (dx <= 0 && dy > 0) {
              // bottom left
              ORI_X = 17;
              ORI_Y = 29;
              cssDeg = Math.round(-90 * qScale);
            } else if (dx > 0 && dy <= 0) {
              // top right
              ORI_X = 33;
              ORI_Y = 40;
            ...
            const arrowKF =
              ...
              "100%{transform:" +
              baseA +
              "rotate(0deg);transform-origin:0px 0px}";
            const pKFcss =
          ...
          state.lastAy = wy1;
          state.lastDur = dur;
          if (window.__vc && window.__vc._overlay && window.__vc._overlayUpdate)
        ...
        window.__vc = {
          ...
          update: function (nx, ny) {
            // backwards compat; returns ms duration
            return moveTo(nx, ny);
          },
          ...
          snapTo: function (nx, ny) {
            ...
            try {
              state.raAnim && state.raAnim.cancel();
            } catch (e) {}
            wrap.style.transform =
              "translate3d(" +
              Math.round(nx) +
          ...
          setMotion: function (p) {
            Object.assign(MOTION, p || {});
            if (
              window.__vc &&
          ...
          dump: function () {
            try {
              ...
              console.debug("[VC/dump]", {
                orbitMode: MOTION.orbitMode,
                orbitMinPx: MOTION.orbitMinPx,
                wrap: tf(state.wrap),
                wrapInner: tf(state.wrapInner),
                pivot: tf(state.pivot),
                arrow: tf(state.arrow),
                badge: tf(state.badge),
                wrapPos: { x: state.wrapX, y: state.wrapY },
                lastDur: state.lastDur,
              });
              return true;
            } catch (e) {
              console.warn("[VC/dump] error", e);
              return false;
            }
          },
          setHover: function (p) {
            Object.assign(HOVER, p || {});
          },
          setOrbitMode: function (m) {
            if (m) MOTION.orbitMode = String(m);
            return MOTION.orbitMode;
          },
          getOrbitMode: function () {
            return MOTION.orbitMode;
          },
            ...
            try {
              if (!flag) {
                if (window.__vc && window.__vc._overlay) {
                  window.__vc._overlay.remove();
                  window.__vc._overlay = null;
                }
                return true;
              }
              if (!window.__vc._overlay) {
                ...
                el.style.cssText =
                  "position:fixed;bottom:8px;left:8px;background:rgba(0,0,0,0.6);color:#fff;padding:6px 8px;border-radius:6px;font:12px/1.4 -apple-system,Segoe UI,Arial;z-index:2147483647;pointer-events:none;white-space:pre;";
              ...
              window.__vc._overlayUpdate = function () {
                try {
                  const txt =
                    ...
                    ")\n" +
                    "current: cssA:" +
                    (state.cssActiveA ? 1 : 0) +
                    " cssB:" +
                    (state.cssActiveB ? 1 : 0) +
                    "\n" +
                    "last dur: " +
                    (state.lastDur || 0) +
                    "ms\n" +
                    "cancels: " +
                    state.cancelLog.length +
                    "\n" +
                    "lastMoveAt: +" +
                    Math.round(state.lastMoveAt || 0) +
                    "ms";
          ...
          testMove: function (dx, dy, opts) {
            try {
              const s = window.__vc && window.__vc._s;
              if (!s) return 0;
              ...
              if (typeof dx !== "number" || typeof dy !== "number") {
                ...
                x = Math.max(
                  20,
                  Math.min(W - 20, Math.round(Math.random() * W)),
                );
                y = Math.max(
                  20,
                  Math.min(H - 20, Math.round(Math.random() * H)),
                );
              }
              return window.__vc.moveTo(x, y, opts || null);
            } catch (e) {
              console.warn("[VC/testMove] error", e);
              return 0;
            }
          },
            ...
            try {
              const nowTS =
                window.performance && performance.now
                  ...
                  : Date.now();
              const rem =
                window.__vc && typeof window.__vc.getSettleMs === "function"
                  ? window.__vc.getSettleMs()
                  : 0;
      ...
      try {
        console.error("[VC] inject error", e);
      } catch (_) {}
      return "error:" + (e && (e.message || e));
    }
  }
  try {
    Object.defineProperty(window, "__vcInstall", {
      value: __vcInstall,
      configurable: true,
      writable: true,
    });
  } catch (_) {
    window.__vcInstall = __vcInstall;
  }
})();
